const items = [
  ...
  { label: "Gaming", icon: <GamingIcon />, badge: "New" },
  { label: "Settings", icon: <SettingsIcon /> },
  { label: "Save post", icon: <SavePostIcon /> },
];

export default function LeftSidebar() {
  return (
    <div className="mb-4 rounded-[6px] bg-surface pt-6 pr-6 pb-1.5 pl-6">
      <h4 className="mb-6 text-[20px] leading-[1.4] font-medium text-heading">
        Explore
      </h4>
      <ul>
        {items.map((item) => (
          <li key={item.label} className="relative mb-6">
            <span className="flex w-full cursor-pointer items-center text-[16px] leading-[1.4] font-medium text-muted hover:text-primary dark:text-white">
              <span className="mr-[14px] flex shrink-0">{item.icon}</span>
              {item.label}
            </span>
            {item.badge && (
              <span className="absolute top-1/2 right-0 flex h-6 w-9 -translate-y-1/2 items-center justify-center rounded-[8px] border-2 border-white dark:border-surface bg-success text-[13px] leading-[1.4] text-white">
                {item.badge}
              </span>
            )}
          </li>
...
function InsightsIcon() {
  return (
    <svg width="22" height="24" viewBox="0 0 22 24" fill="none" className="text-muted">
      <path
        fill="currentColor"
