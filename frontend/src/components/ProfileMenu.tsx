...
export default function ProfileMenu({ user }: { user: User | null }) {
  ...
  return (
    <div ref={ref} className="relative flex h-[68px] items-center">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex cursor-pointer items-center"
      >
        <span className="mr-2 block h-6 w-6 overflow-hidden rounded-full">
          <Image
            src={avatar}
            alt=""
            width={24}
        ...
        <span className="hidden text-[16px] text-heading sm:block">{name}</span>
        <svg className="mt-[-3px] ml-2 text-heading" width="10" height="6" viewBox="0 0 10 6" fill="none">
          <path
            fill="currentColor"
      ...
      {open && (
        <div className="absolute top-[62px] right-0 z-20 w-[312px] rounded-[6px] bg-surface p-4 shadow-[0_10px_20px_rgba(0,0,0,0.08)]">
          <div className="flex items-center">
            <span className="mr-2 block h-[54px] w-[54px] shrink-0 overflow-hidden rounded-full">
              <Image
                src={avatar}
                alt=""
                width={54}
            ...
            <div>
              <h4 className="mb-1 text-[16px] leading-tight font-bold text-heading">
                {name}
              </h4>
              <span className="cursor-pointer text-[14px] text-[#377dff]">
                View Profile
              </span>
            </div>
          </div>
            ...
            {menu.map((item) => (
              <li key={item.label} className="mb-4 last:mb-0">
                <button
                  ...
                  className="flex w-full cursor-pointer items-center justify-between text-[16px] font-medium text-muted hover:text-primary"
                >
                  <span className="flex items-center">
                    <span className="mr-2 inline-flex rounded-full bg-[#ebf2ff] dark:bg-[#1e3a5f] p-[11px]">
                      {item.icon}
                    </span>
                    {item.label}
                  </span>
